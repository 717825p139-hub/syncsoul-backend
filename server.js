        ...
        await fetch("https://api.brevo.com/v3/smtp/email", {
            ...
            body: JSON.stringify({
                ...
                <div style="font-family:Arial;max-width:400px;margin:auto;padding:30px;background:#1a0010;border-radius:15px;color:white;">
                    ...
                    <div style="background:#e63973;color:white;font-size:36px;font-weight:bold;text-align:center;padding:20px;border-radius:10px;letter-spacing:8px;">
                        ${otp}
                    </div>
...
app.post("/api/register", async (req, res) => {
    try {
        ...
        if (password.length < 6) { return res.status(400).json({ error: "Password must be at least 6 characters." }); } const
            emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; if (!emailRegex.test(email)) {
                return res.status(400).json({
                    error: "Invalid email address."
                });
            } const digits = phone.replace(/\D/g, ""); if (digits.length < 10) {
                return
            ...
            hashedPassword = await bcrypt.hash(password, 10); const otp = Math.floor(100000 + Math.random() * 900000).toString();
        otpStore.set(email.toLowerCase(), {
            otp, expires: Date.now() + 5 * 60 * 1000, pendingUser: {
                username, password:
                    hashedPassword, email: email.toLowerCase(), phone, gender, dob, cast, salary, fname, mname, bio, photo: photo || ""
            }
        }); await sendOTPEmail(email, otp); res.json({ message: "OTP sent to your email." });
    } catch (err) {
        console.error("FULL ERROR:", err); res.status(500).json({ error: err.message });
    }
}); app.get("/", (req, res) => {
...
app.post("/api/reset-password", async (req, res) => {
    try {
        ...
        if (!newPassword || newPassword.length < 6) return res.status(400).json({
            error: "Password must be at least 6 characters."
        }); const hashed = await bcrypt.hash(newPassword, 10); await
            User.findOneAndUpdate({ username: entry.username }, { password: hashed }); otpStore.delete(key); res.json({
